...
function pathToEnemy(finder)
{
	...
	PF.pointC.index = pixCoordToIndex(PF.pointC.x, PF.pointC.y, SLVDEngine.currentLevel.layerFuncData[PF.finder.layer]);
	//No parent
	PF.pointC.parent = -1;
	...
	while(cont == 1)
	{
		...
		for(var index = 0; index < SLVDEngine.boardAgent.length; index++)
		{
			...
			if(!currentAgent.getTeam().isAllied(PF.finder.getTeam()))
			{
				//if(index > 100) alert("something is wrong");
				//If reached one opponent's position, set path to troop
				if(PF.pointC.x == xPixToTile(currentAgent.x) && PF.pointC.y == yPixToTile(currentAgent.y))
				{
					//alert("found target");
...
PF.evaluatePoint = function(x, y)
{
	...
	//Get point's index
	var i = pixCoordToIndex(x, y, SLVDEngine.currentLevel.layerFuncData[PF.finder.layer]);
	//If the point has not yet been analyzed and spot on function map is not blocked
	if(PF.point[i] == null && SLVDEngine.currentLevel.layerFuncData[PF.finder.layer].data[i] != 255 && x >= 0 && y >= 0 && x < SLVDEngine.currentLevel.layerFunc[PF.finder.layer].width && y < SLVDEngine.currentLevel.layerFunc[PF.finder.layer].height)
	{
	...
	else if(SLVDEngine.currentLevel.layerFuncData[PF.finder.layer].data[i] == 255 || x < 0 || y < 0 || x >= SLVDEngine.currentLevel.layerFunc[PF.finder.layer].width || y >= SLVDEngine.currentLevel.layerFunc[PF.finder.layer].height) //If point is on closed list or is blocked by terrain
	{ 
	//alert("blocked");
	} 
...
PF.evaluateSquare = function(x, y)
{
	...
	if(done != 1)
	{
		...
		if(SLVDEngine.currentLevel.layerFuncData[PF.person.layer].data[pixCoordToIndex(x, y, SLVDEngine.currentLevel.layerFuncData[PF.person.layer])] != 255 && blocked != 1)
		{
